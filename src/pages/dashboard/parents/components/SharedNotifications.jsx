              ...
              {['all','achievement','improvement','chat'].map(type => (
                ...
                >
                  {type === 'all' ? `الكل (${typeCounts.all})` :
                   type === 'achievement' ? `الإنجازات (${typeCounts.achievement})` :
                   type === 'improvement' ? `التحسينات (${typeCounts.improvement})` :
...
{/* البحث */}
      {showSearch && (
        <div className="my-4">
          <div className="flex items-center bg-gray-200 border border-gray-300 rounded-lg px-2 py-1">
            <FaSearch className="text-gray-400 ml-2" />
            <input
              type="text"
              placeholder="ابحث عن الرسائل، الإنجازات، التحسينات.."
              value={searchTerm}
      ...
      >
        {displayNotifications.map((notification, index) => {
          const bgColor = index % 2 === 0 ? 'bg-white' : 'bg-[#E0F4F5]';
          const { typeLabel, categoryLabel } = getGreenBoxLabels(notification);

          return (
            <motion.div
              key={notification.id}
              className={`${bgColor} rounded-lg border border-gray-200 p-4 cursor-pointer hover:shadow-md transition-shadow`}
  ...
  <span className="font-bold text-sm">{getTypeLabel(notification.type)}</span>
  <span className="bg-[#DCFCE7] text-[#016630] text-xs px-2 py-1 rounded">{typeLabel}</span>
  <span className="bg-[#DCFCE7] text-[#016630] text-xs px-2 py-1 rounded">{categoryLabel}</span>
</div>

              <p className="text-gray-700 text-sm mb-1">{notification.description}</p>
