    ...
    if (file) {
      ...
      img.onload = () => {
        ...
        if (img.width < 100 || img.height < 100) {
          setErrors(prev => ({ ...prev, avatar: 'حجم الصورة صغير جدًا (100×100 على الأقل)' }));
          return;
        }
      ...
      <motion.div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <motion.div className="flex flex-col md:flex-row items-center mb-8 p-6 bg-white dark:bg-gray-700 rounded-lg justify-between" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}>
          <div className="relative mb-4 md:mb-0 md:ml-6">
            <img src={avatarPreview || profile.profileImage} alt="Profile" className="w-24 h-24 rounded-full object-cover border-4 border-white dark:border-gray-600 shadow" />
          ...
          <div className="flex-1 flex flex-col md:flex-row md:items-center justify-between w-full md:ml-6">
            <div className="text-center md:text-right">
              ...
              {errors.avatar && <motion.p className="text-red-500 text-sm mt-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>{errors.avatar}</motion.p>}
            </div>
            ...
            >
              {updating ? <div className="animate-spin rounded-full h-4 w-4 border-t-2 border-b-2 border-white mr-2"></div> : <FaSave className="text-white mr-2" />}
        ...
        <motion.div className="mt-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}>
          <div className="bg-gray-100 dark:bg-gray-700 rounded-lg p-6 border border-gray-300 dark:border-gray-600">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* مدرسة الفاروق */}
              <motion.div className="flex items-center p-4 dark:bg-gray-600 rounded-lg shadow-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 1.1 }} whileHover={{ y: -2 }}>
                <img src="/images/farouq-school.png" alt="مدرسة الفاروق" className="w-16 h-16 rounded-full object-cover border-2 border-white dark:border-gray-600 shadow" />
                <div className="mr-4">
                  <h3 className="font-bold text-gray-900 dark:text-white">مدرسة الفاروق</h3>
                  <div className="flex items-center mt-1">
                    <FaSchool className="text-gray-400 ml-2 text-sm" />
                    <span className="text-gray-600 dark:text-gray-300 text-sm">غزة </span>
                  </div>
                  <div className="flex items-center mt-1">
                    <FaGraduationCap className="text-gray-400 ml-2 text-sm" />
                    <span className="text-gray-600 dark:text-gray-300 text-sm">المرحلة الثانوية</span>
                  </div>
                </div>
              </motion.div>
            </div>
